...
INSERT INTO contact_segment (segmentation_id, segment_id, contact_id, joined_at, left_at, trigger_action_id) -- INSERT Expiring
  SELECT
    @sn, @seg_expiring, contact_id, expiring_start AS joined_at, left_at, trigger_action_id
  FROM (
    SELECT
      m1.contact_id,
      ...
      @last_left_at := CASE WHEN m2.contact_id IS NULL THEN @timeout -- this is the last event
  ...
  WHERE expiring_start != left_at
    AND left_at != 0  -- remove continuations (0 is speacial value and means that contact continues to be in this segment)
