...
INSERT INTO action_page
  ...
  SELECT
    ...
    WHEN activity_type_id = 57 THEN 'consent'
  ...
  WHERE a.activity_type_id IN (2, 3, 32, 54, 57, 59, 67)
-- BEGIN INCREMENTAL
...
-- Create donate action_page for every contribution and contribution recur
INSERT INTO action_page
  (campaign_id, action_type, language, external_id, external_system)
  SELECT
    DISTINCT
  ...
  JOIN ${SOURCE}.civicrm_value_speakout_integration_2 x ON x.entity_id=c.id
-- BEGIN INCREMENTAL
  WHERE c.id NOT IN (SELECT external_id FROM action_page WHERE external_system = 'civicrm_campaign' AND action_type = 'donate')
-- END INCREMENTAL
